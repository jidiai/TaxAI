...
class ppo_agent:
    def __init__(self, envs, args):
        ...
        self.optimizer = optim.Adam(self.households_net.parameters(), self.args.lr, eps=self.args.eps)
        # running filter...
        ...
        for update in range(self.args.n_epochs):
            ...
            for step in range(self.args.epoch_length):
                with torch.no_grad():
                    ...
                    values, pis = self.households_net(self.obs_concate(global_obs_tensor, private_obs_tensor, update=False))
                # select actions
                house_actions = select_actions(pis)
                input_actions = house_actions.copy()
                gov_action = np.array([0, 0, 0, 0, 0.189, 0.4])
                ...
                mb_obs.append(np.copy(self.obs_concate(global_obs_tensor, private_obs_tensor, update=False)))
            ...
            if update % self.args.display_interval == 0:
                print('[{}] Update: {} / {}, Frames: {}, Rewards: {:.3f}, Min: {:.3f}, Max: {:.3f}, PL: {:.3f},'\
                    'VL: {:.3f}, Ent: {:.3f}'.format(datetime.now(), update, self.args.n_epochs, (update + 1)*self.args.nsteps*self.args.num_workers, \
                    final_rewards.mean(), final_rewards.min(), final_rewards.max(), pl, vl, ent))
                # save the model
                if self.args.env_type == 'atari':
                    torch.save(self.households_net.state_dict(), self.model_path + '/model.pt')
                else:
                    # for the mujoco, we also need to keep the running mean filter!
                    torch.save([self.households_net.state_dict(), self.running_state], self.model_path + '/model.pt')
